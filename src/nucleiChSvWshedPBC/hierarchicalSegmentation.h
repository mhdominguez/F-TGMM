...
public:
	...
	*/
	...
	float suggestMerge(TreeNode<nodeHierarchicalSegmentation>* root,  supervoxel& rootSv, TreeNode<nodeHierarchicalSegmentation>** rootMerge,  supervoxel& rootMergeSv, imgTypeC *imgPtr, int debugRecursion = 0);
    ...
    \param in   imgPtr          The image to reference for supervoxel trimming
	\return						Score from [0,1] on probability of being a correct split
	*/
	template<class imgTypeC>
	float suggestSplit(TreeNode<nodeHierarchicalSegmentation>* root,  supervoxel& rootSv, TreeNode<nodeHierarchicalSegmentation>* rootSplit[2],  supervoxel rootSplitSv[2],imgTypeC *imgPtr);
	...
	template<class imgTypeC>
	void cleanHierarchyWithTrimming( unsigned int minSize, unsigned int maxSize, int devCUDA,imgTypeC *imgPtr); //(ngc) deprecated? //calls findRealistic nodes and it also trims supervoxels in order to remove duplicates from hierarchical segmentation
	...
	*/
	template<class imgTypeC>
	void cleanHierarchy(imgTypeC*); 

	//-------------------------debug functions--------------------------------------
	int debugCheckDendrogramCoherence();
	int debugTestMergeSplitSuggestions(imgVoxelType tau,uint16_t *imgPtr);
