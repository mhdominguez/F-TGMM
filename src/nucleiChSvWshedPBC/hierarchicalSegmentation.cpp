...
void hierarchicalSegmentation::cleanHierarchyWithTrimming(unsigned int minSize, unsigned int maxSize, int devCUDA, imgTypeC* imgPtr)
{
	//preset parameters
	...
	{
		iter->trimSupervoxel<imgTypeC>(maxSize,maxPrctile, conn3Dtrim, neighOffsetTrim,imgPtr);
	}
	...
	for(vector<supervoxel>::iterator iter = currentSegmentatioSupervoxel.begin(); iter != currentSegmentatioSupervoxel.end(); ++iter)
	{	
		iter->weightedCentroid<imgTypeC>(imgPtr);//here we only need centroid for the nearest neighbors
	}

	eraseIdx.push_back(0);//just to enter teh for loop
	while( eraseIdx.empty() == false)//some elements could have more than knn_v2_maxKNN to erase
	{
...
//===============================================================================================
template<class imgTypeC>
void hierarchicalSegmentation::cleanHierarchy(imgTypeC *imgPtr)
{
	//preset parameters
	float thrJduplicates = 0.1;//to remove duplicates after trimming
	int conn3DIsNeigh = 74;	
	...
	/* In reality, since maxTau is a variable part of the class, we do not need to lose this information.
	We can just propagate it trhough the code for merge / split. Anything above maxTau is like Infinity
	*/
#if 0
    ...
    void *dataPtr=imgPtr; //basicRegionsVec[0].dataPtr;
	...
	while( q.empty() == false )
	{
		...
		{			
			...
			auxNode->data.svPtr->trimSupervoxel<imgTypeC>(imgPtr);
		}
		//trim also children
		...
		{			
			...
			chL->data.svPtr->trimSupervoxel<imgTypeC>(imgPtr);
		...
		}else if( chL->data.svPtr->tempWildcard > 0.0f )
		{
			...
			chLsv->trimSupervoxel<imgTypeC>(imgPtr);
...
template<class imgTypeC>
float hierarchicalSegmentation::suggestMerge(TreeNode<nodeHierarchicalSegmentation>* root, supervoxel& rootSv, TreeNode<nodeHierarchicalSegmentation>** rootMerge,  supervoxel& rootMergeSv, imgTypeC *imgPtr, int debugRecursion)
{
	...
	*rootMerge = root->parent;
	...
	rootMergeSv.trimSupervoxel<imgTypeC>(imgPtr); //trim supervoxel
	...
	if( rootSv.JaccardDistance( rootMergeSv ) < thrJduplicates )
	{
		...
		*/
		//===========================================
		return suggestMerge<imgTypeC>(rootAux, rootSvAux,  rootMerge, rootMergeSv, imgPtr, debugRecursion + 1);
	}
	...
	sibilingSv.trimSupervoxel<imgTypeC>(imgPtr);//trim supervoxel
...
template<class imgTypeC>
float hierarchicalSegmentation::suggestSplit(TreeNode<nodeHierarchicalSegmentation>* root,  supervoxel& rootSv, TreeNode<nodeHierarchicalSegmentation>* rootSplit[2],  supervoxel rootSplitSv[2],imgTypeC *imgPtr)
{
	const float thrJduplicates = 0.1;//to decide extreme cases
	...
	for( int aa = 0; aa <2; aa++)
	{
		...
		rootSplitSv[aa].trimSupervoxel<imgTypeC>(imgPtr);//trim supervoxel
		...
		if( rootSv.JaccardDistance( rootSplitSv[aa] ) < thrJduplicates )
		{
			...
			return suggestSplit<imgTypeC>(rootAux, rootSvAux, rootSplit, rootSplitSv,imgPtr);
		}
	}
...
//===================================================================================
int hierarchicalSegmentation::debugTestMergeSplitSuggestions(imgVoxelType tau,uint16_t* imgPtr)
{
	...
	{
		//trim supervoxel
		currentSegmentatioSupervoxel[ii].trimSupervoxel<unsigned short int>(imgPtr);

		score = suggestMerge<unsigned short int>(currentSegmentationNodes[ii], currentSegmentatioSupervoxel[ii], &rootMerge, rootMergeSv,imgPtr);

		if( rootMerge != NULL && score > 0)
		{
			//find split and make sure one of the splits is the original root
			suggestSplit<unsigned short int>(rootMerge, rootMergeSv, rootSplit, rootSplitSv,imgPtr);
			...
			if( rootSplit[0] != currentSegmentationNodes[ii] && (rootSplit[1] != currentSegmentationNodes[ii]) )
			{
				...
				for(int aa = 0; aa < 2; aa++)
				{
					...
					svAux.trimSupervoxel<unsigned short int>(imgPtr);
...
template void hierarchicalSegmentation::cleanHierarchyWithTrimming<float>( unsigned int minSize, unsigned int maxSize, int devCUDA,float *imgPtr);
template void hierarchicalSegmentation::cleanHierarchyWithTrimming<unsigned short int>( unsigned int minSize, unsigned int maxSize, int devCUDA,unsigned short* imgPtr);
template void hierarchicalSegmentation::cleanHierarchyWithTrimming<unsigned char>( unsigned int minSize, unsigned int maxSize, int devCUDA,unsigned char* imgPtr);

template void hierarchicalSegmentation::cleanHierarchy<float>(float*);
template void hierarchicalSegmentation::cleanHierarchy<unsigned short int>(unsigned short*);
template void hierarchicalSegmentation::cleanHierarchy<unsigned char>(unsigned char*);

template float hierarchicalSegmentation::suggestMerge<float>(TreeNode<nodeHierarchicalSegmentation>* root,  supervoxel& rootSv, TreeNode<nodeHierarchicalSegmentation>** rootMerge,  supervoxel& rootMergeSv, float *imgPtr, int debugRecursion);
template float hierarchicalSegmentation::suggestMerge<unsigned short int>(TreeNode<nodeHierarchicalSegmentation>* root,  supervoxel& rootSv, TreeNode<nodeHierarchicalSegmentation>** rootMerge,  supervoxel& rootMergeSv, unsigned short int *imgPtr, int debugRecursion);
template float hierarchicalSegmentation::suggestMerge<unsigned char>(TreeNode<nodeHierarchicalSegmentation>* root,  supervoxel& rootSv, TreeNode<nodeHierarchicalSegmentation>** rootMerge,  supervoxel& rootMergeSv, unsigned char *imgPtr, int debugRecursion);

template float hierarchicalSegmentation::suggestSplit<float>(TreeNode<nodeHierarchicalSegmentation>* root,  supervoxel& rootSv, TreeNode<nodeHierarchicalSegmentation>* rootSplit[2],  supervoxel rootSplitSv[2],float *imgPtr);
template float hierarchicalSegmentation::suggestSplit<unsigned short int>(TreeNode<nodeHierarchicalSegmentation>* root,  supervoxel& rootSv, TreeNode<nodeHierarchicalSegmentation>* rootSplit[2],  supervoxel rootSplitSv[2],unsigned short*);
template float hierarchicalSegmentation::suggestSplit<unsigned char>(TreeNode<nodeHierarchicalSegmentation>* root,  supervoxel& rootSv, TreeNode<nodeHierarchicalSegmentation>* rootSplit[2],  supervoxel rootSplitSv[2],unsigned char*);
