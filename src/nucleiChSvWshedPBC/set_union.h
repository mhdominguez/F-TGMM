...
typedef struct {
        labelType *p; 		/* parent element */
        //labelType *size;           /* number of elements in subtree i. Only the root has the right information */ We do not use this throughout the code (what we care about is fMax), so we delete to save memory
        imgVoxelType *fMax;           /* maximum value of the function in subtree i. Only the root has the right information */
        labelType n;				/* number of elements in set */
} set_unionC;
