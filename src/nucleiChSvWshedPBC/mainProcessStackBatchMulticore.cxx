...
int generateBatchScriptFile(string scriptFilename, string ProcessStackPath, string configFilename, int iniFrame, int endFrame, int numCores);
...
int main( int argc, const char** argv )
{
	if( argc != 4 && argc != 5)
	{
		cout<<"ERROR: wrong number of parameters. Call executable with <configFile> <iniFrame> <endFrame> <numCores-optional>"<<endl;
		return 2;
	}
	...
	int err = generateBatchScriptFile(scriptFilename, ProcessStackPath, configFilename, iniFrame, endFrame, numCores);
	if( err > 0 )
		return err;
	...
	//check if files have geen generated (it is equivalent to assume successful execution)
	cout<<"======================================================"<<endl;
	cout<<"======================================================"<<endl;
	cout<<"Checking if hierarchical segmentation files were properly created"<<endl;
	...
	for(int frame = iniFrame; frame <= endFrame; frame++)
	{
		...
		{
			cout<<"====ERROR: hierarchical segmentation file "<<fileOutHS<<" was not created.===="<<endl;
			cout<<"Run the following command to analyze the error: "<<endl;
			cout<<ProcessStackPath<<" \""<< configFilename<<"\" "<<frame<<endl;
			flagError = true;
		}else{
			cout<<"OK: Hierarchical segmentation file "<<fileOutHS<<" was successfully created"<<endl;
		}
		fin.close();
	}
...
int generateBatchScriptFile(string scriptFilename, string ProcessStackPath, string configFilename, int iniFrame, int endFrame, int numCores)
{
	...
	{
		fout<<"ERROR: generateBatchScriptFile: file "<<scriptFilename<<" could not be generated to run batch script"<<endl;
		return 2;
	}
	...
	fout<<":: Display the output of each process if the /O option is used"<<endl;
	...
	fout<<":: List of commands goes here. Each command is prefixed with :::"<<endl;


	for(int ii = iniFrame; ii <= endFrame; ii++)
	{
		fout<<"::: "<<ProcessStackPath<<" \""<< configFilename<<"\" "<<ii<<endl;
	}


	fout<<":: Define the maximum number of parallel processes to run."<<endl;
	fout<<":: Each process number can optionally be assigned to a particular server"<<endl;
	...
	fout<<"::"<<endl;
	fout<<":: If a cpu is not defined for a proc, then it will be run on the local machine."<<endl;
	fout<<":: I haven't tested this feature, but it seems like it should work."<<endl;
	fout<<"::"<<endl;
	...
	fout<<":: For this demo force all cpu specs to undefined (local machine)"<<endl;
	fout<<"for /l %%N in (1 1 %maxProc%) do set \"cpu%%N=\""<<endl;

	fout<<":: Get a unique base lock name for this particular instantiation."<<endl;
	fout<<":: Incorporate a timestamp from WMIC if possible, but don't fail if"<<endl;
	fout<<":: WMIC not available. Also incorporate a random number."<<endl;
	fout<<"set \"lock=\""<<endl;
	fout<<"for /f \"skip=1 delims=-+ \" %%T in ('2^>nul wmic os get localdatetime') do ("<<endl;
	...
	fout<<"for /f \"tokens=* delims=:\" %%A in ('findstr /b \":::\" \"%~f0\"') do ("<<endl;
	...
	fout<<"if defined showOutput echo -------------------------------------------------------------------------------"<<endl;
	fout<<"echo !time! - proc!nextProc!: starting %%A"<<endl;
	fout<<"2>nul del %lock%!nextProc!"<<endl;
	fout<<"%= Redirect the lock handle to the lock file. The CMD process will     =%"<<endl;
	fout<<"%= maintain an exclusive lock on the lock file until the process ends. =%"<<endl;
	fout<<"start /b "" cmd /c %lockHandle%^>\"%lock%!nextProc!\" 2^>^&1 !cpu%%N! %%A"<<endl;
	fout<<")"<<endl;
	...
	fout<<":: redirect stderr to null to suppress any error message if redirection"<<endl;
	fout<<":: within the loop fails."<<endl;
	fout<<"for /l %%N in (1 1 %startCount%) do ("<<endl;
	fout<<"%= Redirect an unused file handle to the lock file. If the process is    =%"<<endl;
	fout<<"%= still running then redirection will fail and the IF body will not run =%"<<endl;
	fout<<"if not defined endProc%%N if exist \"%lock%%%N\" ("<<endl;
	fout<<"%= Made it inside the IF body so the process must have finished =%"<<endl;
	fout<<"if defined showOutput echo ==============================================================================="<<endl;
