	...
	{		
		...
		for(unsigned long long int ii=0;ii<numSamples;ii++)
		{
			...
			while(childNode!=-1)
			{
				...
				if (auxStump->th > 1e30 || xTest[numSamples*(auxStump->featureNdx) + ii] <= auxStump->th)
				{
					childNode = auxStump->leChild;
					if (childNode == -1)//update prediction
					{
	...
	{		
		...
		for(unsigned long long int ii=0;ii<numSamples;ii++)
		{
			...
			while(childNode!=-1)
			{
				...
				if (auxStump->th > 1e30 || xTestAux[auxStump->featureNdx] <= auxStump->th)
				{
					childNode = auxStump->leChild;
					if (childNode == -1)//update prediction
					{
	...
	for(unsigned long long int ii =0;ii<numWeakClassifiers; ii++)
	{
		...
		for(unsigned long long int jj = 0; jj < classifier[ii].size(); jj++)
		{
			out >> 
				classifier[ii][jj].a >>
				classifier[ii][jj].b >>
				classifier[ii][jj].featureNdx >>
				classifier[ii][jj].gtChild >>
				classifier[ii][jj].leChild >>
				classifier[ii][jj].parentIdx >>
				classifier[ii][jj].th;
		}
	}
	out.close();
...
//===============================================================================
int readBinaryTrainingFile(string filename, vector<float>& xTrain, vector<float>& yTrain, long long int &numFeatures, long long int &numSamples)
{
	ifstream  fin(filename.c_str(), ios::binary | ios::in );

	if( fin.is_open() == false )
	{
		cout<<"ERROR: readBinaryTrainingFile: could not open file "<<filename<<" to read training data"<<endl;
		return 2;
	}
	...
	cout<<"Reading "<<numFeatures<<" features per sample for "<< numSamples<<" samples from file "<<filename<<endl;
	...
	//read from file
	fin.read( (char*)(&(xTrain[ xTrainOffset ] )), sizeof( float ) * numFeatures * numSamples );
