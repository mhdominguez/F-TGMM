...
int main( int argc, const char** argv )
{
	...
	{
		fm>>m[ii]>>m[ii+numEllipsoids]>>m[ii+numEllipsoids*2];//coalescent access for GPU
	}
	fm.close();
	...
	for(int ii=0;ii<numEllipsoids;ii++)
	{
		fW>>W[ii]>>W[ii+numEllipsoids]>>W[ii+numEllipsoids*2]>>W[ii+numEllipsoids*3]>>W[ii+numEllipsoids*4]>>W[ii+numEllipsoids*5];//coalescent access for GPU
	}
	fW.close();    

	dims[0] = 587; dims[1] = 1399; dims[2] = 112;
	...
	W = new double[numEllipsoids*dimsImage*(1+dimsImage)/2];
	W[0] =109.83*0.00129047;W[1] = 109.83*3.75697e-006;W[2] = 109.83*0;W[3] = 109.83*0.000632087;W[4] = 109.83*0;W[5] = 109.83*0.0267087;
	...
	*/
	...
	basicEllipticalHaarFeatureVector **fBasic = calculateEllipticalHaarFeatures(m, W,numEllipsoids,im,dims,devCUDA,0);//no symmetry used in this example
	if(fBasic == NULL)
	...
	float* fVec = NULL; 
	calculateCombinationsOfBasicHaarFeatures(fBasic,numEllipsoids,&numHaarFeaturesPerEllipsoid, &fVec);//function allocates fVec if it is NULL
	if(fVec == NULL)
	...
	*/
	...
	cout<<"Program finished calculating Elliptical Haar features normally. It took "<<toc(&timerF)<<" secs for "<<numEllipsoids<<" ellipsoids and "<<numHaarFeaturesPerEllipsoid<<" extended features"<<endl;
	return 0;
}