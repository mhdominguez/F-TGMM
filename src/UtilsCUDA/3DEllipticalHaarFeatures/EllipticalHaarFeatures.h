...
struct basicEllipticalHaarFeatureVector
{
	...
	basicEllipticalHaarFeatureVector()//default constructor
	{
	...
	basicEllipticalHaarFeatureVector(const basicEllipticalHaarFeatureVector& p)//copy constructor (we do not need to copy static variables)
	{
		//allocate memory
		ringAvgIntensity = new float[numRings];
		ringAvgIntensityDoG = new float[numRings];
	...
	basicEllipticalHaarFeatureVector& operator=(const basicEllipticalHaarFeatureVector& p);

	void setToDefault();//reset values to a default state
...
//do not call this function: this is just called from calculateEllipticalHaarFeatures in order to seprate the computation in slices, so we don't run out of memory in large images.
basicEllipticalHaarFeatureVector** calculateEllipticalHaarFeaturesBlockWise(const double *m,const double *W,int numEllipsoids,const imageType *im,const long long int *dims,int symmetry=0);
