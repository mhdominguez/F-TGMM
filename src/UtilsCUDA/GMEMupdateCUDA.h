...
static const int MAX_SUPERVOXELS_PER_GAUSSIAN = 16;//since I do it in the GPU I can not use vector for dynamic allocation
struct GaussianMixtureModelCUDA
{
	...
	//assignment operator
	__device__ GaussianMixtureModelCUDA& operator=(const GaussianMixtureModelCUDA& p);
...
void GMEMvariationalInferenceCUDAWithSupervoxels(float *queryCUDA,float *imgDataCUDA,pxi *rnkCUDA,pxi *rnkCUDAtr,int *indCUDA,int *indCUDAtr,float *centroidLabelPositionCUDA,long long int *labelListPtrCUDA,GaussianMixtureModelCUDA *vecGMtemp,long long int query_nb,int ref_nb,unsigned int maxLabels,int maxIterEM,double tolLikelihood,int devCUDA,int frame, bool W4DOF,string debugPath="");
...
double addTotalLikelihoodWithSupervoxels(float *imgDataCUDA,float *likelihoodVecCUDA,float* finalSumVectorInHostF,GaussianMixtureModelCUDA *vecGMCUDA,int *indCUDA,float *queryCUDA,long long int *labelListPtrCUDA,int query_nb,unsigned int numLabels,double totalAlpha);
