...
{
	//teporalWindowSize = 0;//you have to setup temporal windiw size yourself since it is a static variable
};

backgroundDetectionFeatures::backgroundDetectionFeatures(int teporalWindowSize_)
{
...
//========================================================
int backgroundDetectionFeatures::calculateFeatures(TreeNode< ChildrenTypeLineage >* root, int devCUDA)
{
	const int numKNNdist = 6;//to calculate ratios between nearest neighbors

	f.resize( getNumFeatures() );

	if( root == NULL || root->getNumChildren() != 1) //cell divisions or dying cells are excluded
	{
	...
	{				
		...
		while( auxNode != NULL )
		{
			if ( auxNode->data->TM - iniTM + 1 > teporalWindowSize )//we reached the end of the window size
			{				
				break;
			}
	...
	if( lengthBranchMax < 3 )
		return 3;//not possible to calculate stats
	...
	vector<float> nucOffsetJaccard;//Jaccard index after subtracting translation between nuclei
	...
	for(int ii =0 ; ii < dimsImage; ii++)
		XYZ[ii].reserve( teporalWindowSize );
	...
	while( q.empty() == false )
	{
		...
		if( auxNode->getNumChildren() == 1 && ( auxNode->data->TM - iniTM < teporalWindowSize ) )//we only keep going if there are no cell divisions
		{
			if( auxNode->left != NULL )
				q.push( auxNode->left);
			else
		...
		if( auxNode->parent != NULL )
			nucOffsetJaccard.push_back( lineageHyperTree::JaccardDistance( auxNode, true ) );
		...
		int err2 = lineageHyperTree::findKNearestNucleiNeighborInSpaceEuclideanL2(auxNode->data, iterNucleusNNvec , distVec);//distVec is sorted in ascending order
		if( err2 > 0 )
			return err2;
		for( int ii = 0; ii < numKNNdist; ii++)
		{
	...
	if( nucSize.size() < 3 )
	{
		memset( &(f[0]), 0, sizeof(float) * f.size() );//all features except for temporal ones would be zero		
		numFeatures += 20;
	}
	else{
		...
		for( int ii =0; ii< dimsImage; ii++)
		{
			...
			for(size_t jj = 0; jj < XYZ[ii].size(); jj++ )
				XYZ[ii][jj] -= mm;
		}
		...
		float maxVal = -1e32f;//to remove largest difference (kind of robust metric)
		int maxPos = -1;
		for(size_t jj = 0; jj < U.size(); jj++ )
		{
			...
			U[jj] = max( 0.0, p - lambda2 * U[jj]*U[jj]);//distance from point ot straight line. max() to avoid rounding errors for singular cases (perfect line)
		...
		{
			nucSize[jj] = ( nucSize[jj] - nucSize[jj+1])/ nucSize[jj];//relative nucSize
		}
		nucSize.pop_back();
	...
	//temporal features: based on analyzing branches (so they include cell division)
	//inspired by [1]V. Bettadapura, G. Schindler, T. Plötz, and I. Essa, “Augmenting Bag-of-Words: Data-Driven Discovery of Temporal and Structural Information for Activity Recognition,” in IEEE Conference on Computer Vision and Pattern Recognition (CVPR), 2013.
	int histEvents[3] = {0, 0, 0};//displacement, death, cell division (histogram counting frquency of events)
	int histBranchLengthDeath[3] = {0, 0,0};//accumulates stats of length for branches ended in cell death. The bins are [0,3],[4,6],[7,Inf)
	int histBranchLengthWindow[3] = {0, 0,0};//accumulates stats of length for branches ended with the temporal window.  The bins are [0,3],[4,6],[7,Inf)
	int histBranchLengthDiv[3] = {0, 0,0};//accumulates stats of length for branches ended in cell divisions.  The bins are [0,3],[4,6],[7,Inf)
...
//=======================================================================================
bool backgroundDetectionFeatures::isCorrectConfidenceLevel(TreeNode< ChildrenTypeLineage >* root, int confidenceLevel)
{
	if( ( (int) root->data-> confidence ) != confidenceLevel )
		return false;
