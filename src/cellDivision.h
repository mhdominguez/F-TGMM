...
/*
\brief very simple routine to check for cell division. For all the nuclei in time TM that have more than 1 supervoxel, it checks if it can partition the group of supervoxels into two with max flow / min-cut (i.e, in two sets where supervoxels are not touching)
...
int cellDivisionMinFlowTouchingSupervoxels(lineageHyperTree& lht, int TM, int conn3D, size_t minNeighboringVoxels, int &numCellDivisions, int &numBirths, bool regularizePrecisionMatrix,float* imgPtr);//considers the possibility of split in more than 2 ways
...
//------------------------debug-------------------------
//test KullbackLeibler method for ISBI2013 paper in comparison with EllipticalHaar3Dfeatures
