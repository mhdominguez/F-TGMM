...
#if 0
template <class ItemType>
void BinaryTree<ItemType>::remove_old(TreeNode<ItemType>* root)
{
...
void BinaryTree<ItemType>::remove(TreeNode<ItemType>* root)
{
    ...
    if (root) {
        q.push(root);
        if (root->parent == NULL)
            main_root = NULL; //if the main root is being deleted, main_root must be set to NULL
        else
            current = root->parent; 
    }
    while (!q.empty()) {
...
//=================================================================================================================== 
//The clear() function deletes all nodes in the list. This is very easy to do, since we can take advantage of the
//remove() function, which we has already defined. The remove() functions deletes all nodes of a subtree, as well as
//the root node. Therefore, we can pass the main root to remove() in order to delete all nodes in the tree.
template <class ItemType>
