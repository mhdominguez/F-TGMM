...
struct VoxelFA
{
	...
	friend bool operator< ( VoxelFA& lhs, VoxelFA& rhs);//descending order (we have inverted the order)
};
...
bool supervoxel::sv_keep_without_trimming = false;
...
unsigned int supervoxel::getmaxKNNCUDA()
{
	return knn_v2_maxKNN;
}
...
supervoxel& supervoxel::operator=(const supervoxel& p)
{
	if (this != &p)
	{
		...
		PixelIdxListFull = p.PixelIdxListFull;

		sv_keep_without_trimming = p.sv_keep_without_trimming;

		memcpy(centroid,p.centroid,sizeof(float)*dimsImage);
		memcpy(precisionW,p.precisionW,sizeof(float)*dimsImage * (1+dimsImage) /2);
...
void supervoxel::writeToBinary(ostream& os) const //write ot binary file
{
	os.write((char*) (&TM),sizeof(int));
	os.write((char*) (&dataSizeInBytes), sizeof(uint64));
	os.write((char*) (&dataDims), sizeof(uint64) * dimsImage);
	if (sv_keep_without_trimming)
	{
		unsigned int ll = PixelIdxListFull.size();
		os.write((char*)(&ll), sizeof(unsigned int));
		os.write((char*)(&(PixelIdxListFull[0])), sizeof(uint64)* ll);
	}
	else{
...
/// If buf is NULL this function only computes and returns the required size.
/// If buf is not NULL and n is less than the required size, an exception is thrown.
/// Otherwise, when the buffer is large enough, it is filled with
/// the seriealized supervoxel data.
///
/// \param buf [in] A pointer to a region of memory to fill with data.
/// \param n   [in[ The size of buf in bytes.
/// \return The required size of buf in bytes
unsigned supervoxel::writeToBuffer(char* buf, unsigned n) const {
	if(dimsImage!=3) throw runtime_error("Expected dimsImage to be 3.");
...
#pragma pack(pop)
	...
	const unsigned ll = sv_keep_without_trimming?PixelIdxListFull.size():PixelIdxList.size();	
	const unsigned bytes_required=
		sizeof(sv_header_t) +
		sizeof(uint64)*ll;		  // pixel index list
	if(buf==NULL)
		return bytes_required;
	if(n<bytes_required)
		throw runtime_error("supervoxel::writeToBuffer() was passed a buffer of insufficient size.");

	// assemble
	sv_header_t *sv=(sv_header_t*)buf;
	sv->TM=TM;
	sv->dataSizeInBytes=dataSizeInBytes;
	memcpy(&sv->dims,&dataDims,sizeof(sv->dims));
	sv->npx=ll;

	auto px=sv_keep_without_trimming?(&PixelIdxListFull[0]):(&PixelIdxList[0]);
	memcpy(sv->px,px,ll*sizeof(uint64));
...
{
	cout<<"DEBUGGING: at supervoxel::debugPixelIdxListIsSorted: supervoxel "<<(*this)<<endl;
	if(PixelIdxList.empty())
		return 1;

	uint64 pOld = PixelIdxList[0];
	for(vector<uint64>::iterator iter = PixelIdxList.begin() + 1; iter != PixelIdxList.end();++iter)
	{
		if(pOld >= (*iter))
		{
...
//=============================================================================================================
int supervoxel::neighboringVoxels(const supervoxel &s, int conn3D, int64* neighOffset, vector<uint64> &PixelIdxListBorderThis, vector<uint64> &PixelIdxListBorderS)
{
	//delete memory
	...
	if(s.PixelIdxList.empty())
		return 0;
	if(PixelIdxList.empty())
		return 0;
	if(s.PixelIdxList[0] > PixelIdxList[ PixelIdxList.size()-1 ] + neighOffset[conn3D - 1])
		return 0;
	if((int64)(s.PixelIdxList[ s.PixelIdxList.size() - 1 ]) < (int64) (PixelIdxList[ 0 ]) + neighOffset[0])
		return 0;
	...
	for(int ii = 0; ii<conn3D; ii++)
	{
		...
		while(pos1 < PixelIdxList.size() - 1 && pos2 < s.PixelIdxList.size()-1 )//-1 to avoid if inside when we do posXX++
		{					
	...
	vector<uint64>::iterator iter = unique(PixelIdxListBorderS.begin(), PixelIdxListBorderS.end());
...
//=============================================================================================================
int supervoxel::neighboringVoxels(const supervoxel &s, int conn3D, int64* neighOffset, int satNeighboringVoxels)
{
	cout<<"ERROR: at supervoxel::neighboringVoxels: code not finished!!!"<<endl;
	exit(3);
...
//============================================================================================================
bool supervoxel::isNeighboring(const supervoxel& s, int conn3D, int64* neighOffset)
{
...
{
	cout<<"===WARNING!!::supervoxel::neighboringVoxelsSelf: not tested yet!!!!!======"<<endl;

	const uint64 maxJumpX = 5;//to detect "border"pixels(at least in 2D) and exclude them from the counting
	...
	for(size_t ii =0 ;ii < PixelIdxList.size() - 1; ii++ )
	{
		...
		if(  PixelIdxList[ii] - p > maxJumpX )
			continue;//we are in a border pixel in the supervoxel (it would distor the counting)

		count++;
		for(int jj = 0; jj < conn3D; jj++)
		{
			if( std::binary_search(PixelIdxList.begin(), PixelIdxList.end(), p + neighOffset[jj]) == true )//element found
				mm++;				
		}
	}
...
//==========================================================
//the main idea is to "shoot" lines long X and count gaps. Inspired by how mylib stores arbitrary regions
int supervoxel::numHoles()
{
	const uint64 maxJumpX = dataDims[0] / 3;//to avoid checking coordinates. If the gap is larger than a third the image, we assume we are in the next line for the "line scan" of holes
	uint64 count = 0, aux;
...
//===========================================================================
//returns if idx belongs to pixelIdxList or not. If you have a clue of the position in the list (for example, looking at neighboring elements), search would be faster. We use a simple linear search since the list tends to be small
bool supervoxel::idxBelongsToPixelIdxListLinearSearch(uint64 idx, size_t iniPosSearch)
{
...
//===================================================================
int64* supervoxel::buildNeighboorhoodConnectivity(int conn3D, int64* boundarySize)
{
	...
	for(int ii =0;ii<dimsImage;ii++)
	{
		...
		if( dataDims[ii] <= 0 )
		{
			cout<<"ERROR: supervoxel::buildNeighboorhoodConnectivity: dataDims seem to be wrong (negative or zero ): you need to set them "<<endl;
			exit(3);
		}
	}
	...
	default:
		cout<<"ERROR: at supervoxel::buildNeighboorhoodConnectivity Code not ready for these connected components"<<endl;
		return NULL;
	}
...
void supervoxel::mergeSupervoxels(vector<supervoxel*> &src)
{
	...
	for(int ii = 0;ii<dimsImage;ii++)
			centroid[ii] *= w;
	float aux;
	for(vector<supervoxel*>::iterator iterS = src.begin(); iterS != src.end(); ++iterS)
	{
		...
		{
			cout<<"ERROR: at mergeSupervoxel: dataPtr. We cannot merge supervoxels"<<endl;
			exit(3);
		}
#endif

		//merge Pixeld Idx List
		PixelIdxList.insert( PixelIdxList.end(), (*iterS)->PixelIdxList.begin(), (*iterS)->PixelIdxList.end() );
	...
	//sort merged list. TODO: I could create a merge sort function (merge multiple sorted lists) to gain a little bit of performance.
...
//===================================================================================================================================
//calculates nearest neighbors from supervoxelA (query points) into supervoxelB (reference points). It stores results in supervoxelA[ii].nearestNeighborsInTimeForward
int supervoxel::nearestNeighbors(list< supervoxel > &superVoxelA, list< supervoxel > &superVoxelB, unsigned int KmaxNumNN, float KmaxDistKNN,int devCUDA, vector< vector< SibilingTypeSupervoxel > > &nearestNeighborVec, vector< vector< float > > *nearestNeighborDist2Vec)//you need to setup scale to calculate this property
{
	...
	long long int query_nb = superVoxelA.size();//we find kNN for each query_nb point with respect to ref_nb
	...
	for( list<supervoxel>::iterator iter = superVoxelA.begin(); iter != superVoxelA.end(); ++iter)
	{
		offset = count;
		for(int ii =0; ii<dimsImage; ii++)
		{
	...
	for( list<supervoxel>::iterator iter = superVoxelB.begin(); iter != superVoxelB.end(); ++iter)
	{
		offset = count;
		for(int ii =0; ii<dimsImage; ii++)
		{
...
//===================================================================================================================================
//calculates nearest neighbors from supervoxelA (query points) into supervoxelB (reference points). 
int supervoxel::nearestNeighbors(list< supervoxel > &superVoxelA, vector< supervoxel > &superVoxelB, unsigned int KmaxNumNN, float KmaxDistKNN,int devCUDA, vector< vector< vector<supervoxel>::iterator > > &nearestNeighborVec, vector< vector< float > > *nearestNeighborDist2Vec)//you need to setup scale to calculate this property
{
	supervoxel::setKmaxDistKNN(KmaxDistKNN);
	supervoxel::setKmaxNumNN(KmaxNumNN);
	...
	count = 0, offset;
	vector< vector<supervoxel>::iterator > vecIter(ref_nb);//we will need it later to assign ind to nearest neighbor
	for( vector<supervoxel>::iterator iter = superVoxelB.begin(); iter != superVoxelB.end(); ++iter)
	{
		offset = count;
		for(int ii =0; ii<dimsImage; ii++)
		{
...
//===================================================================================================================================
//calculates nearest neighbors from supervoxelA (query points) into supervoxelB (reference points). x_i = supervoxelB[0][i] 
int supervoxel::nearestNeighbors(vector< float > superVoxelA[dimsImage], vector< float > superVoxelB[dimsImage], unsigned int KmaxNumNN, float KmaxDistKNN,int devCUDA, vector< vector< int > > &nearestNeighborVec, vector< vector< float > > *nearestNeighborDist2Vec )//you need to setup scale to calculate this property
{
	supervoxel::setKmaxDistKNN(KmaxDistKNN);
	supervoxel::setKmaxNumNN(KmaxNumNN);
	...
	long long int query_nb = superVoxelA[0].size();//we find kNN for each query_nb point with respect to ref_nb
	...
	for(int ii =0; ii<dimsImage; ii++)
	{
		for( vector<float>::iterator iter = superVoxelA[ii].begin(); iter != superVoxelA[ii].end(); ++iter)
		{			
			ref_xyz[count] = *iter;		
			count++;
		}
	...
	for(int ii =0; ii<dimsImage; ii++)
	{
		for( vector<float>::iterator iter = superVoxelB[ii].begin(); iter != superVoxelB[ii].end(); ++iter)
		{			
			ref_xyz[count] = *iter;		
			count++;
		}
...
//===================================================================================================================================
//calculates nearest neighbors from supervoxelA (query points) into supervoxelB (reference points). x_i = supervoxelB[0][i] 
int supervoxel::nearestNeighbors(vector< supervoxel* >& superVoxelA, vector< float > superVoxelB[dimsImage], unsigned int KmaxNumNN, float KmaxDistKNN,int devCUDA, vector< vector< int > > &nearestNeighborVec, vector< vector< float > > *nearestNeighborDist2Vec )//you need to setup scale to calculate this property
{
	supervoxel::setKmaxDistKNN(KmaxDistKNN);
	supervoxel::setKmaxNumNN(KmaxNumNN);
	...
	//copy centroids to temporary array
	long long int count = 0, offset;
	for( vector<supervoxel*>::iterator iter = superVoxelA.begin(); iter != superVoxelA.end(); ++iter)
	{
		offset = count;
		for(int ii =0; ii<dimsImage; ii++)
		{
...
//===================================================================================================================================
//calculates nearest neighbors from supervoxelA (query points) into supervoxelB (reference points). 
int supervoxel::nearestNeighbors(vector< supervoxel > &superVoxelA, vector< supervoxel > &superVoxelB, unsigned int KmaxNumNN, float KmaxDistKNN,int devCUDA, vector< vector< vector<supervoxel>::iterator > > &nearestNeighborVec, vector< vector< float > > *nearestNeighborDist2Vec)//you need to setup scale to calculate this property
{
	supervoxel::setKmaxDistKNN(KmaxDistKNN);
	supervoxel::setKmaxNumNN(KmaxNumNN);
...
int supervoxel::weightedGaussianStatistics(double* m, double *W, float* intensity, bool regularizeW,const imgTypeC* imgPtr)
{
	...
	{
		for(vector<uint64>::const_iterator iterP = PixelIdxList.begin(); iterP != PixelIdxList.end(); ++iterP)
		{
			coordAux = (*iterP);
...
//===================================================================================================
template<class imgTypeC>
int supervoxel::weightedGaussianStatistics(bool regularizeW,const imgTypeC* imgPtr)
{
	//imgTypeC* imgPtr = (imgTypeC*)(dataPtr);
	float imgVal, N_k = 0.0f;
	int64 coordAux;
	...
	if( precisionW != NULL)
	{
		...
		if( N_k < 1e-5 )//special case to avoid segmentation fault
		{
			...
			for(int ii = 0; ii<dimsImage; ii++)
			{
				for(int jj = ii; jj <dimsImage; jj++)
				{
					precisionW[count] = precisionW[count] / N_k - centroid[ii] * centroid[jj]; 
					count++;
				}
			}
		}
...
//===================================================================================================
template<class imgTypeC>
int supervoxel::weightedCentroid(imgTypeC* imgPtr)
{
...
//=====================================================================
template<class imgTypeC>
int supervoxel::robustGaussianStatistics(double*m, double* W, int maxSize, float maxPercentile,imgTypeC* imgPtr)
{
	...
	//1.-calculate optimal threshold for voxels based on three criteria: maxSize, maxPercentile and Otzu threshold
	...
	float I;
	sv.weightedGaussianStatistics<imgTypeC>(m, W, &I, false,imgPtr);//we should not need to regularize
}

//====================================================================
template<class imgTypeC>
imgTypeC supervoxel::intensityThreshold(int maxSize, float maxPercentile, unsigned int *maxPosPixelIdxList, bool localBackgroundSubtracted,imgTypeC* imgPtr)
{
...
//===========================================================================
//inspired in code from: http://www.labbookpages.co.uk/software/imgProc/otsuThreshold.html
//Note: you can calculate what is called the between class variance, which is far quicker to calculate. 
//Luckily, the threshold with the maximum between class variance also has the minimum within class 
//variance. So it can also be used for finding the best threshold and therefore due to being simpler 
//is a much better approach to use.
...
//====================================================================
template<class imgTypeC>
imgTypeC supervoxel::trimSupervoxel(int maxSize, float maxPercentile,int conn3D, int64* neighOffset,imgTypeC* imgPtr)
{

	//make copy before trimming
	if (sv_keep_without_trimming)
	{		
		PixelIdxListFull = PixelIdxList;
	}
	...
	{
		removeZeros<imgTypeC>(imgPtr);//remove all background elements
	}
	...
	//calculate threshold	
	thr = intensityThreshold<imgTypeC>(maxSize, maxPercentile, &maxPosPixelIdxList, localBackgroundSubtracted,imgPtr);
	...
	while(qPos.empty() == false)
	{
		...
		for(int jj = 0;jj<conn3D; jj++)
		{
			...
			if( iterPos != PixelIdxList.end() && (*iterPos) == idxNeigh )//element found
			{
	...
	for(size_t ii = 0;ii<PixelIdxList.size();ii++)
	{
		if( visited[ii] > 1 )
		{
			PixelIdxListAux[newSize++] = PixelIdxList[ii];//we keep them alreayd sorted
		}
	}
...
float supervoxel::mergePriorityFunction(supervoxel& svCh1, supervoxel& svCh2)
{
	...
	//simple test: are they touching?check if both supervoxels are actually touching (otherwise we do not merge)
	if( svCh1.isNeighboring( svCh2 , pMergeSplit.conn3DIsNeigh, pMergeSplit.neighOffsetIsNeigh) == false )
	{
	...
	{
		//cout<<"mergePriority 0.0f because deltaZ = "<<getDeltaZ()<<">"<<pMergeSplit.deltaZthr<<endl;
		return 0.0f;
	}

	//cout<<"WARNING: supervoxel::mergePriorityFunction NOT DONE YET!!!!!"<<endl; //TODO
...
//============================================================
template<class imgTypeC>
void supervoxel::removeZeros(imgTypeC* ptr)
{
	int pos = 0;
	for(int ii = 0; ii < PixelIdxList.size(); ii++)
	{
		...
		{
			//PixelIdxList.erase( PixelIdxList.begin() + ii );//I don't do swap and resize to preserve sorting. BUT this is ridiculously slow
...
float supervoxel::Mahalanobis2Distance(const supervoxel& s) const
{
	...
	for(int ii=0;ii<dimsImage; ii++)
	{
		aux[ii] = (centroid[ii]-s.centroid[ii]);//I do not need to scale anything because W already scales
	}
...
template float supervoxel::trimSupervoxel<float>(int maxSize, float maxPercentile, int conn3D, int64* neighOffset,float* imgPtr);
template unsigned short int supervoxel::trimSupervoxel<unsigned short int>(int maxSize, float maxPercentile, int conn3D, int64* neighOffset,unsigned short* imgPtr);
template unsigned char supervoxel::trimSupervoxel<unsigned char>(int maxSize, float maxPercentile, int conn3D, int64* neighOffset,unsigned char* imgPtr);

template float supervoxel::trimSupervoxel<float>(float*);
template unsigned short int supervoxel::trimSupervoxel<unsigned short int>(unsigned short*);
template unsigned char supervoxel::trimSupervoxel<unsigned char>(unsigned char*);

template void supervoxel::removeZeros<float>(float*);
template void supervoxel::removeZeros<unsigned short int>(unsigned short*);
template void supervoxel::removeZeros<unsigned char>(unsigned char*);


template float supervoxel::intensityThreshold<float>(int maxSize, float maxPercentile, unsigned int *maxPosPixelIdxList, bool localBackgroundSubtracted,float*);
template unsigned short int supervoxel::intensityThreshold<unsigned short int>(int maxSize, float maxPercentile, unsigned int *maxPosPixelIdxList, bool localBackgroundSubtracted,unsigned short*);
template unsigned char supervoxel::intensityThreshold<unsigned char>(int maxSize, float maxPercentile, unsigned int *maxPosPixelIdxList, bool localBackgroundSubtracted,unsigned char*);

template int supervoxel::weightedGaussianStatistics<float>(double*m, double* W,float* intensity, bool regularizeW,const float*);
template int supervoxel::weightedGaussianStatistics<unsigned short int>(double*m, double* W,float* intensity, bool regularizeW,const unsigned short*);
template int supervoxel::weightedGaussianStatistics<unsigned char>(double*m, double* W,float* intensity, bool regularizeW,const unsigned char*);


template int supervoxel::weightedGaussianStatistics<float>(bool regularizeW,const float*);
template int supervoxel::weightedGaussianStatistics<unsigned short int>(bool regularizeW,const unsigned short*);
template int supervoxel::weightedGaussianStatistics<unsigned char>(bool regularizeW,const unsigned char*);

template int supervoxel::weightedCentroid<float>(float*);
template int supervoxel::weightedCentroid<unsigned short int>(unsigned short*);
template int supervoxel::weightedCentroid<unsigned char>(unsigned char*);

template int supervoxel::robustGaussianStatistics<float>(double*m, double* W, int maxSize, float maxPercentile,float*);
template int supervoxel::robustGaussianStatistics<unsigned short int>(double*m, double* W, int maxSize, float maxPercentile,unsigned short*);
template int supervoxel::robustGaussianStatistics<unsigned char>(double*m, double* W, int maxSize, float maxPercentile,unsigned char*);


template float supervoxel::mergePriorityFunction<float>(supervoxel& svCh1, supervoxel& svCh2);
template float supervoxel::mergePriorityFunction<unsigned short int>(supervoxel& svCh1, supervoxel& svCh2);
template float supervoxel::mergePriorityFunction<unsigned char>(supervoxel& svCh1, supervoxel& svCh2);
