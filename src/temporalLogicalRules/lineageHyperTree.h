...
using TimeSeriesMapT = map<int, mylib::Array*>;
...
public:
	...
	void setFrameAsT_o(unsigned int TM);//deletes anything < TM. All the nuclei existing at TM are set as root of the starting lineages. Does not deallocates associated image data for TM-1 in the time_series_map
	static size_t getNucleusVolume(nucleus &nuc);
	...
	int readListSupervoxelsFromTifWithWeightedCentroid(string filenameImage,string filenameLabels,int vecPosTM,TimeSeriesMapT &time_series_map);
	int readBinaryRnkTGMMfile(string filenameRnk,int vecPosTM);//reads Rnk binary file to associate supervoxels into nuclei
	int parseTGMMframeResult(vector<GaussianMixtureModelRedux*> &vecGM, int vecPosTMm,  string svBinaryFilename = "");//uses vecRefLastTreeNode to keep track of where each new nuclei needs to be connected to 
	...
	int writeListSupervoxelsToBinaryFile(string filename, unsigned TM) const;
	int readListSupervoxelsFromBinaryFile(const string filename, unsigned int TM);//reads svb binary file into supervoxelsList[TM]
	...
	int calculateNucleiIntensityCentroid(SibilingTypeNucleus& iterN,imgTypeC* imgPtr);//recalculates centroid based on intensity. If data is not present then it is just a mean of coordinate
	
	template<class imgTypeC>
	void calculateNucleiGMMParameters(SibilingTypeNucleus& iterN, bool regularizeW, const float scale[dimsImage],imgTypeC* imgPtr);//recalculates centroids and precision matrix based on intensity.

	template<class imgTypeC>
	void calculateGMMparametersFromNuclei(list<nucleus>::iterator &iterN, float* m_k, float *N_k, float* S_k,imgTypeC* imgPtr);//calculates statistics for the Gaussian based on supervoxls belonging to centroid. We assume r_nk =1

	//metrics
	static float JaccardDistance(TreeNode<ChildrenTypeLineage>* node, TreeNode<ChildrenTypeLineage>* node2, bool applyCentroidOffset); //Jaccard distance between two nuclei (aggregates all supervoxels). If applyCentroidOffset == trie, then we apply translation offset using centroids
	static float JaccardDistance(TreeNode<ChildrenTypeLineage>* node, bool applyCentroidOffset); //node2 = node->parent
	...
	*/
	...
	int mergeShortLivedDaughters(TreeNode<ChildrenTypeLineage>* rootSplit, int lengthsTMthr,const TimeSeriesMapT &time_series_map);//for each cell division, checks when any of the the daughters die. If it is below lengthTMthr, then it undoes the split and merges both daughters 
	int mergeShortLivedDaughtersAll(int lengthTMthr, int maxTM, int& numCorrections, int& numSplits,const TimeSeriesMapT &time_series_map);// checks for all possible corrections in *this. maxTM in order to avoid conisdering dead the "forefront" of tracking. Just set maxTM = maxINT to ignore this constraint
	...
	int deleteShortLivedLineages(TreeNode<ChildrenTypeLineage>* rootBirth, int lengthTMthr, bool removeLineage);
	int deleteShortLivedLineagesAll(int TM, int lengthTMthr, int& numCorrections, int& numBirths);//checks all the lineages that are borned at TM and deletes the ones with length < lengthTMthr	
	...
	int mergeNonSeparatingDaughters(TreeNode<ChildrenTypeLineage>* rootSplit,int conn3D, int64* neighOffset,const TimeSeriesMapT &time_series_map);//for each cell division, checks the distance over time between the two daughters. If they are always neighboring, it merges them.
	int mergeNonSeparatingDaughters(TreeNode<ChildrenTypeLineage>* rootSplit,int conn3D, int64* neighOffset, size_t minNeighboringVoxels,const TimeSeriesMapT &time_series_map);
	int mergeNonSeparatingDaughtersAll(int maxTM, size_t minNeighboringVoxels, int conn3D, int& numCorrections, int& numSplits,const TimeSeriesMapT &time_series_map);
	...
	void cutLinkageBetweenMotherAndFurthestDaughter(TreeNode<ChildrenTypeLineage> *motherNode);
	...
	*/
	...
	int mergeBranches(TreeNode<ChildrenTypeLineage>* root1 ,TreeNode<ChildrenTypeLineage>* root2,const TimeSeriesMapT& time_series_map);//merges two branches (all the way until one dies) starting at rootX (root1 and root2 are also merged)
	...
	void debugPrintLineageForLocalLineageDisplayinMatlab(TimeSeriesMapT & time_series_map,string imgPath, string imgLPath, string suffix = "", string imgRawPath = "");
	void debugPrintLineageToMCMCxmlFile(string filename,string imgPrefix, string imgSuffix);
	void debugCanvasFromSegmentationCroppedRegion(unsigned int TM, TimeSeriesMapT & time_series_map, uint64 minXYZ[dimsImage], uint64 maxXYZ[dimsImage],string imgBasename, string imgRawPath ="");
	...
	int debugCheckOddPrecisionMatrix(int TM, const string& note);
...
//==============================================
inline size_t lineageHyperTree::getNucleusVolume(nucleus &nuc)
{
	size_t vol = 0;		

	for( vector<ChildrenTypeNucleus>::iterator iter = nuc.treeNode.getChildren().begin(); iter != nuc.treeNode.getChildren().end(); ++iter)
	{
		vol += (*iter)->PixelIdxList.size();
	}
