...
public:
	...
	float probCellDivision;//score from classifier for possible cell division
	...
	float Euclidean2Distance(const double s[dimsImage], float scale[dimsImage] ) const;
...
//=================================================
inline float nucleus::Euclidean2Distance(const double s[dimsImage], float scale[dimsImage]) const
{
	float dist = 0;
	float aux = 0;
	for(int ii=0;ii<dimsImage; ii++)
	{
		aux = (centroid[ii]-s[ii]) * scale[ii];
		dist += aux*aux;
	}
	return dist;
}
